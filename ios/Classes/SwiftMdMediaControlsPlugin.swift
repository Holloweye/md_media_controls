...
public class SwiftMdMediaControlsPlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: FlutterResult) {
        ...
        case "play":
            ...
            playerItemObserver = playerItem?.observe(\.status, options: [.new, .old], changeHandler: { [weak self] (playerItem, change) in
                ...
                if (startPosition != 0.0) {
                    ...
                    playerItem.seek(to: seekTime, toleranceBefore: CMTime.zero, toleranceAfter: CMTime.zero, completionHandler: { [weak self]
                        (_: Bool) -> Void in
                ...
                self?.lastTimeRangeDuration = Int(playerItem.duration.seconds)
            ...
            NotificationCenter.default.addObserver(self, selector:#selector(self.audioRouteChanged), name: AVAudioSession.routeChangeNotification, object: nil)
        ...
        case "seek":
            ...
            if let tt = playerItem {
                ...
                tt.seek(to: seekTime, toleranceBefore: CMTime.zero, toleranceAfter: CMTime.zero, completionHandler: { [weak self]
                    (_: Bool) -> Void in
    ...
    func isHeadphones(in routeDescription: AVAudioSessionRouteDescription) -> Bool {
        return !routeDescription.outputs.filter({$0.portType == AVAudioSession.Port.headphones}).isEmpty
    }
    ...
    @objc func startAppTimeObserver(channel: FlutterMethodChannel) {
        lastProgressTime = 0
        playerTimeObserver = player.addPeriodicTimeObserver(forInterval: CMTimeMakeWithSeconds(0.05, preferredTimescale: Int32(NSEC_PER_SEC)), queue: nil) { [weak self]
            time in
